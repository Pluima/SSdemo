# Copyright 2023 Zhongqiu Wang
# Apache-2.0 license http://www.apache.org/licenses/LICENSE-2.0
# Modified from https://github.com/espnet/espnet
...
class av_TFGridNetV3(AbsSeparator):
    """Offline TFGridNetV3.
    ...
    def __init__(self,args):
        ...
        self.direction_proj = nn.Sequential(
            nn.Linear(2*3, args.network_reference['emb_size']),
            nn.ReLU(),
            nn.Linear(args.network_reference['emb_size'], args.network_reference['emb_size']),
        )
        self.av_conv = _clones(nn.Linear(emb_dim+ args.network_reference['emb_size'], emb_dim),n_layers)

    def forward(self, input, vector):
        ...
        ilens = [input.shape[1] for i in range(input.shape[0])]
        ilens = torch.Tensor(ilens).int()

        mix_std_ = torch.std(input, dim=(1, 2), keepdim=True)  # [B, 1, 1]
        input = input / (mix_std_ + 1e-9)  # RMS normalization
        ...
        direction_emb = self.direction_proj(vector) # [B, emb_size]
        direction_emb = direction_emb[..., None, None] # [B, emb_size, 1, 1]
        direction_emb = direction_emb.repeat(1, 1, n_frames, n_freqs) # [B, emb_size, T, F]

        for ii in range(self.n_layers):
            batch = torch.cat((direction_emb,batch), axis=1)
