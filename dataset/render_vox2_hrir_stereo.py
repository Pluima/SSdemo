#!/usr/bin/env python3
"""Render VoxCeleb2 2mix rows into binaural stereo mixtures with HRIR."""
...
def load_hrir_bank(mat_path: Path, prefer_style: str) -> HrirBank:
    if loadmat is None:
        raise RuntimeError(
            "scipy is required to read .mat HRIR files. "
            "Install it first, e.g. `pip install scipy`."
        )
    ...
    # Expected dims: [taps, dirs, channels].
    if hrir.ndim != 3:
    ...
    if hrir.shape[1] != azimuth.shape[0]:
        raise ValueError(
            f"HRIR direction dim mismatch: M_data {hrir.shape}, M_directions {directions.shape}"
        )
...
def pick_direction_pair(
    ...
    candidates = np.where(np.isclose(elevation_deg, chosen_elev))[0]
    if len(candidates) < 2:
        candidates = np.arange(len(azimuth_deg))

    idx1 = int(rng.choice(candidates))
    valid = [i for i in candidates if angle_distance_deg(float(azimuth_deg[i]), float(azimuth_deg[idx1])) >= min_sep_deg]
    if not valid:
        valid = [i for i in candidates if int(i) != idx1]
    if not valid:
        raise RuntimeError("Not enough HRIR directions to pick 2 sources")
    idx2 = int(rng.choice(valid))
    return idx1, idx2, float(azimuth_deg[idx1]), float(azimuth_deg[idx2]), chosen_elev
...
def main():
    parser = argparse.ArgumentParser(
        description="Render VoxCeleb2 2mix CSV into binaural stereo mixes with OlHeaD HRIR."
    )
    parser.add_argument(
        "--mix-csv",
        type=Path,
    ...
    parser.add_argument(
        "--out-root",
        type=Path,
        default=Path("/home/qysun/Neuro-SS/dataset/VoxCeleb2-mix-hrir-stereo"),
        help="Output root for generated stereo mixtures.",
    )
    parser.add_argument(
        "--out-csv",
        type=Path,
        default=Path("/home/qysun/Neuro-SS/dataset/VoxCeleb2-mix-hrir-stereo/mix_sources_vox2_hrir.csv"),
        help="Output CSV (sound-bubble online format).",
    )
    ...
    parser.add_argument("--min-az-sep", type=float, default=40.0, help="Min azimuth separation between 2 speakers.")
    ...
    with args.out_csv.open("w", newline="", encoding="utf-8") as f:
        ...
        for row_idx, row in enumerate(iter_vox2_rows(args.mix_csv)):
            ...
            try:
                ...
                d1, d2, az1, az2, elev = pick_direction_pair(
                    hrir_bank.azimuth_deg,
                    hrir_bank.elevation_deg,
